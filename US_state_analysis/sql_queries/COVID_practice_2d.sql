/*Objective: create a (pivot) table where each row represents a different date between '2020-01-22' and '2022-10-18', each column represents a different US state category -- Rep-held governorship, Dem-held governorship, or flipped governorship -- and the cells contain a rolling 7-day average of COVID cases per 100K residents.*/
                          ...
                          FROM
                               (
                                SELECT A.state1 state__,
                                       ...
                                       CASE WHEN A.state1 IN ('CA', 'CO', 'CT', 'DE', 'HI', 'IL', 'KS', 'KY', 'LA', 'ME', 'MI', 'MN', 'NC', 'NJ', 'NJ', 'NM', 'NV', 'NY', 'OR', 'PA', 'RI', 'VA', 'WA', 'WI') THEN 'Democrat_Held'
                                            WHEN A.state1 IN ('AK', 'AL', 'AR', 'AZ', 'FL', 'GA', 'IA', 'ID', 'IN', 'MA', 'MD', 'MO', 'MS', 'ND', 'NE', 'NH', 'OH', 'OK', 'SC', 'SD', 'TN', 'TX', 'UT', 'VT', 'WV', 'WY') THEN 'Republican_Held'
                                            WHEN A.state1 IN ('MT') THEN 'Flipped'
                                          END AS States1,
            ...
            INNER JOIN (
                        SELECT B2.States2,
                          ...
                          FROM
                               (
                                SELECT B.state2,
                                       CASE WHEN B.state2 IN ('CA', 'CO', 'CT', 'DE', 'HI', 'IL', 'KS', 'KY', 'LA', 'ME', 'MI', 'MN', 'NC', 'NJ', 'NJ', 'NM', 'NV', 'NY', 'OR', 'PA', 'RI', 'VA', 'WA', 'WI') THEN 'Democrat_Held'
                                            WHEN B.state2 IN ('AK', 'AL', 'AR', 'AZ', 'FL', 'GA', 'IA', 'ID', 'IN', 'MA', 'MD', 'MO', 'MS', 'ND', 'NE', 'NH', 'OH', 'OK', 'SC', 'SD', 'TN', 'TX', 'UT', 'VT', 'WV', 'WY') THEN 'Republican_Held'
                                            WHEN B.state2 IN ('MT') THEN 'Flipped'
                                          END AS States2,
