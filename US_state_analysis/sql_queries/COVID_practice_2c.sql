/*Objective: create a (pivot) table where each row represents a different date between '2020-01-22' and '2022-10-18', each column represents a different US state category -- Rep-held legislature, Dem-held legislature, or other (split/flipped) -- and the cells contain a rolling 7-day average of COVID cases per 100K residents.*/
                          ...
                          FROM
                               (
                                SELECT A.state1 state__,
                                       ...
                                       CASE WHEN A.state1 IN ('ME', 'NH', 'VT', 'CT', 'MA', 'RI', 'NY', 'NJ', 'CA', 'CO', 'DE', 'HI', 'IL', 'MD', 'NM', 'NV', 'OR', 'WA') THEN 'Democrat_Held'
                                            WHEN A.state1 IN ('AL', 'AR', 'AZ', 'FL', 'GA', 'ID', 'IN', 'KS', 'KY', 'LA', 'MI', 'MO', 'MS', 'MT', 'NC', 'OH', 'WI', 'IA', 'NE', 'ND', 'NH', 'OK', 'PA', 'SC', 'SD', 'TN', 'TX', 'UT', 'WI', 'WV', 'WY') THEN 'Republican_Held'
                                            WHEN A.state1 IN ('AK', 'MN', 'VA') THEN 'Other'
                                          END AS States1,
            ...
            INNER JOIN (
                        SELECT B2.States2,
                          ...
                          FROM
                               (
                                SELECT B.state2,
                                       CASE WHEN B.state2 IN ('ME', 'NH', 'VT', 'CT', 'MA', 'RI', 'NY', 'NJ', 'CA', 'CO', 'DE', 'HI', 'IL', 'MD', 'NM', 'NV', 'OR', 'WA') THEN 'Democrat_Held'
                                            WHEN B.state2 IN ('AL', 'AR', 'AZ', 'FL', 'GA', 'ID', 'IN', 'KS', 'KY', 'LA', 'MI', 'MO', 'MS', 'MT', 'NC', 'OH', 'WI', 'IA', 'NE', 'ND', 'NH', 'OK', 'PA', 'SC', 'SD', 'TN', 'TX', 'UT', 'WI', 'WV', 'WY') THEN 'Republican_Held'
                                            WHEN B.state2 IN ('AK', 'MN', 'VA') THEN 'Other'
                                          END AS States2,
