/*Objective: create a (pivot) table where each row represents a different date between '2020-01-22' and '2022-10-18', each column represents a different US state category -- Biden-won in 2020, Trump-won in 2020, or both -- and the cells contain a rolling 7-day average of COVID cases per 100K residents.*/
                          ...
                          FROM
                               (
                                SELECT A.state1 state__,
                                       ...
                                       CASE WHEN A.state1 IN ('AZ', 'CA', 'CO', 'CT', 'DE', 'GA', 'HI', 'IL', 'MA', 'MD', 'MI', 'MN', 'NH', 'NJ', 'NM', 'NV', 'NY', 'OR', 'PA', 'RI', 'VA', 'VT', 'WA', 'WI') THEN 'Biden_Won'
                                            WHEN A.state1 IN ('AK', 'AL', 'AR', 'FL', 'IA', 'ID', 'IN', 'MO', 'MS', 'MT', 'NC', 'ND', 'NE', 'NH', 'OH', 'OK', 'SC', 'SD', 'TN', 'TX', 'UT', 'WV', 'WY') THEN 'Trump_Won'
                                            WHEN A.state1 IN ('ME', 'NE') THEN 'Both'
                                          END AS States1,
            ...
            INNER JOIN (
                        SELECT B2.States2,
                          ...
                          FROM
                               (
                                SELECT B.state2,
                                       CASE WHEN B.state2 IN ('AZ', 'CA', 'CO', 'CT', 'DE', 'GA', 'HI', 'IL', 'MA', 'MD', 'MI', 'MN', 'NH', 'NJ', 'NM', 'NV', 'NY', 'OR', 'PA', 'RI', 'VA', 'VT', 'WA', 'WI') THEN 'Biden_Won'
                                            WHEN B.state2 IN ('AK', 'AL', 'AR', 'FL', 'IA', 'ID', 'IN', 'MO', 'MS', 'MT', 'NC', 'ND', 'NE', 'NH', 'OH', 'OK', 'SC', 'SD', 'TN', 'TX', 'UT', 'WV', 'WY') THEN 'Trump_Won'
                                            WHEN B.state2 IN ('ME', 'NE') THEN 'Both'
                                          END AS States2,
