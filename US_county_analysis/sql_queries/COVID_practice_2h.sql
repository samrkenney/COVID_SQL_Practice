  ...
  FROM
	   (
		SELECT x.County_ County_,
			   ...
			   PERCENTILE_DISC(0.025) WITHIN GROUP (ORDER BY CAST(x.Total_COVID_Deaths/x.County_pop*10000 AS decimal(7,2)) ASC) OVER (PARTITION BY x.NCHS_Code) AS lower_percentile_,
			   PERCENTILE_DISC(0.975) WITHIN GROUP (ORDER BY CAST(x.Total_COVID_Deaths/x.County_pop*10000 AS decimal(7,2)) ASC) OVER (PARTITION BY x.NCHS_Code) AS upper_percentile_
		  FROM 
			   (
