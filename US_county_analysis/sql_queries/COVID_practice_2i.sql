...
PERCENTILE_DISC(0.25) WITHIN GROUP (ORDER BY CAST(x.Total_COVID_Deaths/x.County_pop*10000 AS decimal(7,2)) ASC)
OVER (PARTITION BY x.NCHS_Code) AS lower_percentile_,
PERCENTILE_DISC(0.75) WITHIN GROUP (ORDER BY CAST(x.Total_COVID_Deaths/x.County_pop*10000 AS decimal(7,2)) ASC)
